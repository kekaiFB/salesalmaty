var timeFormat = (function (){
    return function (ms){
        ...
        if (Math.abs(sec)<10)
        {
            return  parseFloat('0.0' + seconds);
        } else if (Math.abs(sec)<60) {
            return  parseFloat('0.' + seconds);
        } else {
            if (Math.abs(seconds)<10)
            {
                return parseFloat(minutes + ".0" + seconds);
...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            // Texts
            text:{
                noRecordsFound: 'Нет результатов',
                showingPage: 'Показана страница {0} из {1} существующих',
                show: 'показать ',
                search: 'поиск',
                entries: ' записи',
                ...
                paste: 'Вставить...',
                about: 'О технологии',
                // areYouSureToDeleteTheSelectedRows: 'Вы действительно хотите ужадить текущую строку?',
            ...
            if (obj.formula[cellId] && obj.formula[cellId].length > 0) {
                if (chainLoopProtection[cellId]) {
                    ...
                    for (var i = 0; i < obj.formula[cellId].length; i++) {
                        ...
                        // Update cell
                        if (typeof obj.options.data[cell[1]] !== 'undefined'){
                            if (typeof obj.options.data[cell[1]][cell[0]] !== 'undefined'){
            ...
            var execute = function(expression, x, y) {
             // Parent column identification
                ...
                if (tokens && tokens.indexOf(parentId) > -1) {
                    ...
                    // Convert formula to javascript ТУТ может быть
                    try {
                        Object.keys(formulaExpressions).forEach((key, index) => {
                            if (formulaExpressions[key] == 0) {
                                // Оставляем нули без изменений
                            } else if (!formulaExpressions[key].toString().includes('.')) {
                                var date = new Date(0, 0, 0, 0, Math.abs(formulaExpressions[key].toString().split('.')[0]), 0);
                                var seconds = date.getHours() * 3600 + date.getMinutes() * 60 + date.getSeconds() * 10;
                                formulaExpressions[key] = formulaExpressions[key] < 0 ? -seconds : seconds;
                            } else if (formulaExpressions[key].toString().split('.')[1].length == 1) {
                                var date = new Date(0, 0, 0, 0, Math.abs(formulaExpressions[key].toString().split('.')[0]), formulaExpressions[key].toString().split('.')[1]);
                                var seconds = date.getHours() * 3600 + date.getMinutes() * 60 + date.getSeconds() * 10;
                                formulaExpressions[key] = formulaExpressions[key] < 0 ? -seconds : seconds;
                            } else {
                                var date = new Date(0, 0, 0, 0, Math.abs(formulaExpressions[key].toString().split('.')[0]), formulaExpressions[key].toString().split('.')[1]);
